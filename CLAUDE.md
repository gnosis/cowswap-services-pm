# PySolver playground on Polygon fork
...
4. Keep `CHAIN=137` (required for CoW backend compatibility in this setup).
...
- Use Polygon token addresses (do not reuse Ethereum mainnet token addresses).
- Rabby works better than MetaMask for this local setup because MetaMask blocks adding a local RPC with chain ID `137`.
